...
void DEMO_Render(double deltatime)
{
	...
	// draw arms
	Draw_Polygon2D(&arm1[1], framebuffer, framebuffer_pitch);
	Draw_Polygon2D(&arm2[1], framebuffer, framebuffer_pitch);

	// draw the title
	Draw_Text_GDI("Forward Kinematic DEMO, Press <ESC> to Exit.", 10, 10, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
	Draw_Text_GDI("<A>, <S> to adjust Arm 1, <D>, <F> to adjust Arm 2", 10, 25, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap8bit, "assets/armgrid.bmp");
