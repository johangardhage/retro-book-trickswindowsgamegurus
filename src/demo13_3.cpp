...
void DEMO_Render(double deltatime)
{
	...
	// draw the black hole
	Draw_BOB(&black_hole, framebuffer, framebuffer_pitch);
	...
	// draw the title
	Draw_Text_GDI("GRAVITY MASS DEMO - Use Arrows to Control Ship.", 10, 10, RGB(0, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Friction: X=%f, Y=%f", fx, fy);
	Draw_Text_GDI(buffer, 10, 420, RGB(0, 255, 0), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Velocity: X=%f, Y=%f", ship.varsF[0], ship.varsF[1]);
	Draw_Text_GDI(buffer, 10, 440, RGB(0, 255, 0), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap8bit, "assets/gravsky8.bmp");
	...
	Load_Bitmap_File(&bitmap8bit, "assets/blaze8.bmp");

	// create bob
	Create_BOB(&ship, 320, 240, 22, 18, 32, BOB_ATTR_MULTI_FRAME | BOB_ATTR_VISIBLE, 0);
	...
	Load_Bitmap_File(&bitmap8bit, "assets/photon8.bmp");

	// create bob
	Create_BOB(&black_hole, 32 + rand() % (screen_width - 64), 32 + rand() % (screen_height - 64),
		44, 44, 7, BOB_ATTR_MULTI_FRAME | BOB_ATTR_VISIBLE, 0);
