...
void DEMO_Render(double deltatime)
{
	...
	// draw the title
	Draw_Text_GDI("ELASTIC Object-Object Collision Response DEMO, Press <ESC> to Exit.", 10, 10, RGB(255, 255, 255), framebuffer, framebuffer_pitch);

	// draw the title
	sprintf(buffer, "Coefficient of Restitution e=%f, use <RIGHT>, <LEFT> arrow to change.", cof_E);
	Draw_Text_GDI(buffer, 10, 30, RGB(255, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Total System Kinetic Energy Sum(1/2MiVi^2)=%f ", 0.5 * sqrt(total_ke_x * total_ke_x + total_ke_y * total_ke_y));
	Draw_Text_GDI(buffer, 10, 465, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
