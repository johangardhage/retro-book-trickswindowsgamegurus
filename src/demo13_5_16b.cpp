...
void DEMO_Render(double deltatime)
{
	...
	// draw the puck
	Draw_BOB16(&puck, framebuffer, framebuffer_pitch);

	// draw the title
	Draw_Text_GDI16("(16-Bit Version) SIMPLE FRICTION DEMO - Hit Space to Shoot Puck, Arrows to Change Friction.", 10, 10, RGB16Bit(0, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Friction: X=%f, Y=%f", fx, fy);
	Draw_Text_GDI16(buffer, 10, 410, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Velocity: X=%f, Y=%f", puck.varsF[0], puck.varsF[1]);
	Draw_Text_GDI16(buffer, 10, 430, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Frictional Coefficient: %f", friction);
	Draw_Text_GDI16(buffer, 10, 450, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap16bit, "assets/hockey24.bmp");
	...
	Load_Bitmap_File(&bitmap16bit, "assets/puck24.bmp");

	// create bob
	Create_BOB(&puck, PUCK_START_X, PUCK_START_Y, 32, 32, 1, BOB_ATTR_SINGLE_FRAME | BOB_ATTR_VISIBLE, 0, 0, 16);
