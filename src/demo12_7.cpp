...
void Draw_Ants(void)
{
	...
	for (index = 0; index < NUM_ANTS; index++) {
		...
		Draw_BOB(&ants[index], framebuffer, framebuffer_pitch);
		ants[index].x += 8; ants[index].y += 8; // fix center

		// draw a little number above ant
		sprintf(buffer, "%d", index);
		Draw_Text_GDI(buffer, ants[index].x, ants[index].y - 16, RGB(0, 255, 0), framebuffer, framebuffer_pitch);
...
void Draw_Food(void)
{
	...
	for (index = 0; index < NUM_MNMS; index++) {
		if (food[index].energy > 0) {
			...
			Draw_BOB(&mnm, framebuffer, framebuffer_pitch);
		} // end if
...
#define DELTA_LINE 14
	...
	for (int index = 0; index < NUM_ANTS; index++) {
		...
		if (index < 8) {
			...
			Draw_Text_GDI(buffer, px, py, RGB(0, 255, 0), framebuffer, framebuffer_pitch);

			// print counters
			sprintf(buffer, "C1:%d,C2:%d", ants[index].counter_1, ants[index].counter_2);
			Draw_Text_GDI(buffer, px, py + DELTA_LINE, RGB(0, 255, 0), framebuffer, framebuffer_pitch);
			...
			Draw_Text_GDI(buffer, px, py + 2 * DELTA_LINE, RGB(0, 255, 0), framebuffer, framebuffer_pitch);
			...
			if (ants[index].varsI[ANT_INDEX_AI_STATE] != ANT_DEAD) {
				for (int x = 0; x < 16; x++)
					for (int y = 0; y < 16; y++) {
						...
						Draw_Pixel(px + 124 + x + 1, py + y + 1, intensity_to_index[color_intensity], framebuffer, framebuffer_pitch);

						int active_pixel = ants_mem[index].ilayer[x][y];

						if (active_pixel)
							// scan input layer to see where degradation and communication have taken place
							Draw_Pixel(px + 124 + x + 1, py + y + 1, active_pixel, framebuffer, framebuffer_pitch);
					} // end for
				...
				// draw flat line :)
			{
				HLine(px + 124, px + 124 + 18, py + 9, 249, framebuffer, framebuffer_pitch);
			} // end else
...
void DEMO_Render(double deltatime)
{
	...
	Draw_Text_GDI(buffer, 8, 8, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap8bit, "assets/sidewalk28.bmp");
	...
	Load_Bitmap_File(&bitmap8bit, "assets/antimg8.bmp");

	// create master ant
	Create_BOB(&ants[0], 320, 200, 24, 24, 9, BOB_ATTR_MULTI_ANIM | BOB_ATTR_VISIBLE, 0);
	...
	// create the mnm
	Create_BOB(&mnm, 0, 0, 8, 8, 1, BOB_ATTR_SINGLE_FRAME | BOB_ATTR_VISIBLE, 0);
