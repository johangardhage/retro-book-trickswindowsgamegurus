	...
	for (int index = 0; index < NUM_PROJECTILES; index++) {
		...
		if (missiles[index].state == 1) {
			Draw_Rectangle(missiles[index].x - 1, missiles[index].y - 1,
				...
				249, framebuffer, framebuffer_pitch);
		} // end if
...
void DEMO_Render(double deltatime)
{
	...
	// draw the title
	Draw_Text_GDI("Trajectory DEMO, Press <ESC> to Exit.", 10, 10, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
	Draw_Text_GDI("<RIGHT>, <LEFT> to adjust velocity, <UP>, <DOWN> to adjust angle", 10, 25, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
	Draw_Text_GDI("<G>, <B> to adjust gravity, <W>, <E> to adjust wind, <CTRL> to fire.", 10, 40, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap8bit, "assets/tankgrid.bmp");
