...
void DEMO_Render(double deltatime)
{
	...
	for (int index_y = start_map_y; index_y <= end_map_y; index_y++) {
		for (int index_x = start_map_x; index_x <= end_map_x; index_x++) {
			...
			// draw the texture
			Draw_BOB16(&textures, framebuffer, framebuffer_pitch);
	...
	// draw some info
	Draw_Text_GDI16("(16-Bit Version) USE ARROW KEYS TO MOVE, <ESC> to Exit.", 8, 8, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(buffer, "World Position = [%d, %d]     ", world_x, world_y);
	Draw_Text_GDI16(buffer, 8, screen_height - 32 - 24, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap16bit, "assets/scrolltextures_24.bmp");
