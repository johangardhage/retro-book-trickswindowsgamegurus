...
int Set_Palette_Entry(int color_index, LPPALETTEENTRY color)
{
	...
	RETRO_SetColor(color_index, color->peRed, color->peGreen, color->peBlue);
		...
		for (int index = 0; index < 256; index++) {
			...
			if (lights[index].state == -1) {
				...
				if (++lights[index].counter >= lights[index].off_time) {
					...
					// update color
					RETRO_SetColor(lights[index].color_index, lights[index].on_color.peRed, lights[index].on_color.peGreen, lights[index].on_color.peBlue);
			...
			for (int xx = 0; xx < RETRO_FONT_WIDTH; xx++) {
				if (glyph[yy] & (1 << xx)) {
					...
					if (xpos >= 0 && xpos < screen_width && ypos >= 0 && ypos < screen_height) {
						dest_buffer[ypos * lpitch + xpos] = color;
					}
				}
			}
		}
...
int T3DLIB_Init(int lookup_format = DD_PIXEL_FORMAT565)
{
	if (lookup_format == DD_PIXEL_FORMAT565) {
		RETRO_InitFrameBuffer(SDL_PIXELFORMAT_INDEX8);
...
int T3DLIB_Init32(int pixel_format = DD_PIXEL_FORMATALPHA888)
{
	...
	framebuffer = RETRO.framebuffer;
	framebuffer_pitch = screen_width * 4;
