...
void Draw_Ants(void)
{
	...
	for (index = 0; index < NUM_ANTS; index++) {
		...
		Draw_BOB16(&ants[index], framebuffer, framebuffer_pitch);
		ants[index].x += 8; ants[index].y += 8; // fix center

		// draw a little number above ant
		sprintf(buffer, "%d", index);
		Draw_Text_GDI16(buffer, ants[index].x, ants[index].y - 16, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
...
void Draw_Food(void)
{
	...
	for (index = 0; index < NUM_MNMS; index++) {
		if (food[index].energy > 0) {
			...
			Draw_BOB16(&mnm, framebuffer, framebuffer_pitch);
		} // end if
...
void Draw_Info(void)
{
	...
	static const char *state_to_text[6] = { "Wander", "Eating", "Resting", "Srchfood", "Talking", "Dead" };
...
#define DELTA_LINE 14
	...
	for (int index = 0; index < NUM_ANTS; index++) {
		...
		if (index < 8) {
			...
			Draw_Text_GDI16(buffer, px, py, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);

			// print counters
			sprintf(buffer, "C1:%d,C2:%d", ants[index].counter_1, ants[index].counter_2);
			Draw_Text_GDI16(buffer, px, py + DELTA_LINE, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
			...
			Draw_Text_GDI16(buffer, px, py + 2 * DELTA_LINE, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);

			// draw memory, the brighter the green dot, the stronger the memory
			Draw_Rectangle(px + 124, py, px + 124 + 18, py + 18, 0, framebuffer, framebuffer_pitch);
			...
			if (ants[index].varsI[ANT_INDEX_AI_STATE] != ANT_DEAD) {
				for (int x = 0; x < 16; x++)
					for (int y = 0; y < 16; y++) {
						...
						Draw_Pixel16(px + 124 + x + 1, py + y + 1, RGB16Bit((color_intensity * 16), 0, 0), framebuffer, framebuffer_pitch);

						int active_pixel = ants_mem[index].ilayer[x][y];

						if (active_pixel)
							// scan input layer to see where degradation and communication have taken place
							Draw_Pixel16(px + 124 + x + 1, py + y + 1, RGB16Bit(0, active_pixel, 0), framebuffer, framebuffer_pitch);
					} // end for
				...
				// draw flat line :)
			{
				HLine16(px + 124, px + 124 + 18, py + 9, RGB16Bit(255, 0, 0), framebuffer, framebuffer_pitch);
			} // end else
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap16bit, "assets/sidewalk224.bmp");
	...
	Load_Bitmap_File(&bitmap16bit, "assets/antimg24.bmp");

	// create master ant
	Create_BOB(&ants[0], 320, 200, 24, 24, 9, BOB_ATTR_MULTI_ANIM | BOB_ATTR_VISIBLE, 0, 0, 16);
	...
	// clone the ants
	for (int index = 1; index < NUM_ANTS; index++)
	...
	// create the mnm
	Create_BOB(&mnm, 0, 0, 8, 8, 1, BOB_ATTR_SINGLE_FRAME | BOB_ATTR_VISIBLE, 0, 0, 16);
