...
void DEMO_Render(double deltatime)
{
	...
	for (int index = 0, visible = 0; index < NUM_OBJECTS_UNIVERSE; index++) {
		...
		if ((x0 >= screen_width) || (x1 < 0) || (y0 >= screen_height) || (y1 < 0))
			...
			Draw_BOB16(object_ptr, framebuffer, framebuffer_pitch);
	...
	// draw some info
	Draw_Text_GDI16("(16-Bit Version) USE ARROW KEYS TO MOVE, <ESC> to Exit.", 8, 8, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);

	sprintf(buffer, "World Position = [%d, %d]  Objects Visible = %d, Objects Clipped = %d   ", world_x, world_y, visible, 256 - visible);
	Draw_Text_GDI16(buffer, 8, screen_height - 32, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap16bit, "assets/gens4_24.bmp");
	...
	Load_Bitmap_File(&bitmap16bit, "assets/aliens0_24.bmp");
	...
	Load_Bitmap_File(&bitmap16bit, "assets/pipe0_24.bmp");
