		...
		for (int ball_b = ball_a + 1; ball_b < NUM_BALLS; ball_b++) {
			...
			if (length <= 2.0 * (BALL_RADIUS * .75)) {
				...
				Draw_Clip_Line16(balls[ball_a].varsF[INDEX_X] + 0.5,
					...
					RGB16Bit(0, 255, 255), framebuffer, framebuffer_pitch);
...
void DEMO_Render(double deltatime)
{
	...
	// draw table
	HLine16(TABLE_MIN_X, TABLE_MAX_X, TABLE_MIN_Y, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
	HLine16(TABLE_MIN_X, TABLE_MAX_X, TABLE_MAX_Y, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
	VLine16(TABLE_MIN_Y, TABLE_MAX_Y, TABLE_MIN_X, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
	VLine16(TABLE_MIN_Y, TABLE_MAX_Y, TABLE_MAX_X, RGB16Bit(0, 255, 0), framebuffer, framebuffer_pitch);
	...
	// draw the title
	Draw_Text_GDI16("(16-Bit Version) ELASTIC Object-Object Collision Response DEMO, Press <ESC> to Exit.", 10, 10, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);

	// draw the title
	sprintf(buffer, "Coefficient of Restitution e=%f, use <RIGHT>, <LEFT> arrow to change.", cof_E);
	Draw_Text_GDI16(buffer, 10, 30, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(buffer, "Total System Kinetic Energy Sum(1/2MiVi^2)=%f ", 0.5 * sqrt(total_ke_x * total_ke_x + total_ke_y * total_ke_y));
	Draw_Text_GDI16(buffer, 10, 465, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);
