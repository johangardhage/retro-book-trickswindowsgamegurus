...
void Draw_Waypoints(int mode = 1)
{
	...
	for (int index = 0; index < NUM_WAYPOINTS; index++) {
		...
		Draw_Pixel(path[index].x, path[index].y, 250, framebuffer, framebuffer_pitch);
		Draw_Pixel(path[index].x + 1, path[index].y, 250, framebuffer, framebuffer_pitch);
		Draw_Pixel(path[index].x, path[index].y + 1, 250, framebuffer, framebuffer_pitch);
		Draw_Pixel(path[index].x + 1, path[index].y + 1, 250, framebuffer, framebuffer_pitch);
...
void DEMO_Render(double deltatime)
{
	...
	if (ai_on) {
		...
		if (sign_cross > 0.3) {
			...
			Draw_Text_GDI(buffer, 320, 460, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
	...
	// draw the car
	Draw_BOB(&car, framebuffer, framebuffer_pitch);

	// show nearest waypoint
	sprintf(buffer, "Nearest Waypoint %d", Find_Nearest_Waypoint(car.varsF[0], car.varsF[1]));
	Draw_Text_GDI(buffer, 64, 460, RGB(255, 255, 255), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap8bit, "assets/racetrack.bmp");
	...
	Load_Bitmap_File(&bitmap8bit, "assets/simimg.bmp");

	// create car bob
	Create_BOB(&car, 334, 64, 16, 16, 16, BOB_ATTR_MULTI_FRAME | BOB_ATTR_VISIBLE, 0);
