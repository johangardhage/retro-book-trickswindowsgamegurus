...
void DEMO_Render(double deltatime)
{
	...
	for (int index = NUM_CHOPPERS - 1; index >= 0; index--)
		Draw_Scaled_BOB16(&chopper[index], chopper[index].varsI[0], chopper[index].varsI[1], framebuffer, framebuffer_pitch);

	// draw the title
	Draw_Text_GDI16("(16-Bit Version) CONSTANT VELOCITY DEMO", 10, 10, RGB16Bit(0, 200, 200), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap16bit, "assets/skyscroll24.bmp");
	...
	// load the bitmaps
	Load_Bitmap_File(&bitmap16bit, "assets/chop24.bmp");

	// create bob
	Create_BOB(&chopper[0], 0, 120, 200, 64, 3, BOB_ATTR_MULTI_FRAME | BOB_ATTR_VISIBLE, 0, 0, 16);
